...
你可以告诉面试官你想这样做，然后画一个简单的想法图。画出你的主要组件以及它们之间的连接。如果你这样做，很快就能得到反馈，知道你是否在正确的方向上前进。当然，你必须能够证明你刚刚画的高层设计是合理的。

不要被吸引去深入研究抽象设计的某个特定方面。现在还不是时候。相反，确保你画出重要的组件和它们之间的连接。在面试官面前证明你的想法，并尝试解决每个约束条件和用例。

通常，这种高层设计是人们已经开发出的成熟技术的组合。你需要确保熟悉已有的技术并且能自如地运用这些知识。在本章中，我们假设你有足够的经验来设计这样的高层系统。我们的目标是更多地关注后续步骤，主要讨论可扩展性和消除瓶颈。

如果你觉得自己在系统设计方面是完全的初学者，也许先看一本关于这个主题的书或利用我们在本课程各部分中分享的一些资源会非常有用。
