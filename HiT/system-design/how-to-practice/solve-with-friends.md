...
- Twitter：关注的人的推文的实时动态
- Google：即时返回匹配任何搜索查询的页面（搜索）；存储和服务大量视频数据（YouTube）；按主题聚合全世界的新闻（Google新闻）
- Facebook：服务大量照片

首先列出10到20个这样的问题。然后选一个，尝试提出一个高效的设计。确保遵循前面章节讨论的流程。

准备好后，找一个有知识的朋友或同事，去一个会议室开始讨论想法。这会非常有趣，你会从这个过程中学到很多。

不要花太多时间：最初，每个问题限制在一小时，最终尝试将每个问题控制在20到30分钟内。

如果你找不到朋友或同事，可以去像HighScalability这样的网站，上面有大量真实架构的例子，把你构想的方案与这些公司实际做的进行比较。你会惊讶于有多少顶级网站的架构是公开可用的。请记住，这些系统是由多人花数月时间设计的，所以不要期望能想出他们做的每一个底层决策。专注于高层设计。
