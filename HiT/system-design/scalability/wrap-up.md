...
Finally, you'd be well served throughout your professional career to stay up to date on how scalability evolves. While AWS didn't change the fundamental scalability principles, it did change the landscape of technologies people need to be familiar with when scaling. Thus, staying up to date is quite important.
...
Most of all, have fun. Dreaming up architectures is a very stimulating mental process - enjoy it and stay positive.
...
如果你看了那些真实架构，你会发现很少有一种完美的方式来做事情。每家公司最终都会有不同的架构。设计一个可扩展的系统是一项优化任务：有大量的约束（时间、预算、知识、复杂性、当前可用的技术等），你需要构建最适合这些约束的方案。每种技术、每种模式都在某些方面很好，在其他方面不太好。理解这些优缺点是关键。

记住：没有唯一最优的系统设计。

当然，你可以使用最佳实践。但归根结底，一切都归结为在上市时间、系统复杂性、开发成本、维护成本、可用性以及许多其他因素之间取得平衡。

能够理解和讨论这些权衡——这就是系统设计（以及系统设计题目）的全部内容。

在你的准备中，不要试图找到万能药。相反，专注于每种可扩展性模式擅长什么、它的不足是什么，以及人们为什么更喜欢它而不是其他模式。


## 整合所有知识并保持更新

在这一点上，你能做的最有用的事情是总结出一两页关于你所学到的可扩展性经验。

最后，在你的整个职业生涯中，关注可扩展性如何发展会让你受益匪浅。虽然AWS没有改变基本的可扩展性原则，但它确实改变了人们在扩展时需要熟悉的技术格局。因此，保持更新是相当重要的。


## 在面试中

首先，遵循系统设计流程。不要跳过步骤，不要做假设，从宏观开始，在被要求时深入细节。

其次，记住系统设计问题是一个思想交流的平台。准备好讨论权衡、优缺点。准备好提供替代方案、提出问题、识别和解决瓶颈，根据面试官的偏好进行宏观或深入的讨论。

不要采取防御态度：每当面试官质疑你的架构选择时，承认很少有一个想法是完美的，并概述你选择的优缺点。对讨论中可能出现的新约束保持开放态度，并即时调整你的架构。

最重要的是，享受过程。构想架构是一个非常刺激的思维过程——享受它并保持积极的心态。


## 总结

你现在知道如何应对任何系统设计问题了。你首先通过识别约束条件和用例来构建高层架构，画出主要组件及其之间的关系，思考系统的瓶颈。然后在系统约束的背景下应用正确的可扩展性模式来处理这些瓶颈。
